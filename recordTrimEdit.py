...
pg.mixer.pre_init(
    config["SAMPLE_RATE"], 32, 1, config["SAMPLE_RATE"] // config["CHUNK_RATE"]
)
pg.init()
...
def getEdges():
    ...
    while (
        loudness_bytes(sound_chunks[start_pointer]) <= config["THRESHOLD"]
        and start_pointer < len(sound_chunks) - 1
    ):
        start_pointer += 1
    end_pointer = max(0, len(sound_chunks) - m)
    while (
        loudness_bytes(sound_chunks[end_pointer - 1]) <= config["THRESHOLD"]
        and end_pointer >= 1
    ):
        end_pointer -= 1
...
def drawTranscript(screen):
    ...
    infos = [
        ["Left: reject snippet", "Down: listen to snippet", "Right: approve snippet"],
        [
            "Enter: Instantly save (at the end)",
            "Left-left: Delete previous snippet",
            "Left-down: Listen to previous snippet",
        ],
            ...
            if event.key == keys[1]:  # listen to audio
                ...
                if listening_edges[1] > listening_edges[0]:
                    ...
                    for i in range(listening_edges[0], listening_edges[1]):
                        audio_temp = np.append(
                            audio_temp, np.frombuffer(sound_chunks[i], dtype=np.int16)
                        )
...
for i in range(len(sound_chunks)):
    audio_full[i * chunk_size : i * chunk_size + chunk_size] = np.frombuffer(
        sound_chunks[i], dtype=np.int16
    )
